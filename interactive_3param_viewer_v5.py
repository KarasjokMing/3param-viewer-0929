...
if __name__ == "__main__":
    try:
        ...
        # 将完整的、详细的错误堆栈信息打印到标准输出（这将显示在 Streamlit 日志中）
        print("--- APPLIKASI MENGALAMI KESALAHAN FATAL ---")
        traceback.print_exc()
        print("-----------------------------------------")
        
        # （可选）在 Streamlit 界面上也显示一个错误信息
        st.error("应用遇到一个致命错误，导致其停止运行。请检查日志获取详细信息。")
        st.exception(e)
